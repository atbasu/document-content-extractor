...
There are two ways to fine tune the adapter:
1.
2.
...
**[openai] section:**
- `api_key`: Specifies the API key to be used for OpenAI services. Replace "YOUR_API_KEY_HERE" with the actual API key.
- `model`: Sets the model to be used by OpenAI. In this case, "text-davinci-003" is selected.
- `retry_delay`: Defines the delay (in seconds) between retry attempts when there is a failure. Currently set to 1 second.
- `max_retries`: Indicates the maximum number of retry attempts in case of failure. Currently set to 3.

**[extractor] section:**
- `file_folder`: Specifies the folder where the sample forms are located.
...
- `parser_config`: Specifies the JSON file that contains the configuration for the Closewise parser.

**[prompt_eng] section:**
- `prefix`: Defines the prefix text to be used when generating prompts for extracting field values from the closing confirmation text.
...
- `splits`: Indicates the number of splits to divide the closing confirmation text into when generating prompts.
- `max_tokens`: Defines the maximum number of tokens allowed for a single prompt.
- `response_tokens`: Sets the number of tokens reserved for the response generated by the model.
- `prompt_threshold`: Specifies the threshold (in tokens) for the prompt length. Prompts below this length will be ignored.
- `stopwords`: Specifies the file containing stopwords, which are words to be excluded from the prompt generation process.
