...
def format_for_closewise():
    ...
    if not isinstance(data, dict):
        return jsonify({'error': f'Invalid data format for variable data. Expected JSON object got {type(data)}.'}), 400
    ...
    try:
        ...
        # log an error message
        error_msg_with_traceback = f"An error occurred while processing the data: {str(e)}\n\n{traceback.format_exc()}"
        app.logger.error(error_msg_with_traceback)
        return jsonify(error=error_msg_with_traceback), 500
